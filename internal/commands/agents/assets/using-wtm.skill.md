...
- **`wtm list --output json`** — flat inventory of worktrees.
- **`wtm tree --output json`** — the worktree **forest**: each worktree as a node with its `children`, so the `parent → child` stacking (recorded `source_branch`) is explicit. Use this over `list` when you need to reason about orchestration order (what to rebase first, what blocks what). Each node carries `branch`, `path` (empty for a **virtual** unmanaged-parent node, `is_virtual: true`), `is_main`, `children[]`, and a `status` object: `commits_ahead`, `is_dirty`, **`needs_sync`** (the parent moved past this node — it should be rebased; the key orchestration signal), `in_cycle` (a broken `source_branch` cycle — rendered, never fatal), and `pr` (null unless `--with-prs`). The top-level shape is `{ "roots": [ … ] }`. Flags: `--with-prs` (fetch PR info across **all** states — open/merged/closed — so merged/closed PRs surface as clean candidates; network call, opt-in), `--output text|json|mermaid`. `mermaid` emits a `flowchart TD` to paste into a PR/Notion. `commits_ahead` is measured against the configured base branch (same as `wtm list`). A pure inventory still uses `wtm list`; reach for `tree` when the hierarchy matters.
