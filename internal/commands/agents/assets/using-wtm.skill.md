...
| Worktree **forest** (parent→child tree + needs-sync) | `wtm tree --output json` |
...
- **`wtm list --output json`** — flat inventory of worktrees. Each entry carries `branch`, `path`, `is_parent`, `is_dirty`, `rebase_in_progress` (a rebase is paused mid-way in the worktree — e.g. left by `wtm sync --keep-conflict`), `commits_ahead`, `created_at`, `pr`, and `services`.
- **`wtm tree --output json`** — the worktree **forest**: each worktree as a node with its `children`, so the `parent → child` stacking (recorded `source_branch`) is explicit. Use this over `list` when you need to reason about orchestration order (what to rebase first, what blocks what). Each node carries `branch`, `path` (empty for a **virtual** unmanaged-parent node, `is_virtual: true`), `is_main`, `children[]`, and a `status` object: `commits_ahead`, `is_dirty`, `rebase_in_progress` (a rebase is paused mid-way in the worktree — e.g. left by `wtm sync --keep-conflict`; finish it with `git rebase --continue`/`--abort`), **`needs_sync`** (the parent moved past this node — it should be rebased; the key orchestration signal), `in_cycle` (a broken `source_branch` cycle — rendered, never fatal), and `pr` (null unless `--with-prs`). The top-level shape is `{ "roots": [ … ] }`. Flags: `--with-prs` (fetch PR info across **all** states — open/merged/closed — so merged/closed PRs surface as clean candidates; network call, opt-in), `--output text|json|mermaid`. `mermaid` emits a `flowchart TD` to paste into a PR/Notion. `commits_ahead` is measured against the configured base branch (same as `wtm list`). A pure inventory still uses `wtm list`; reach for `tree` when the hierarchy matters.
- **`wtm create <branch> --from <base> --output json`** — create a new worktree, run env provisioning + `on_create` hooks. `--from` accepts a **local branch or a remote-tracking ref** like `origin/develop` (branch off a remote branch you haven't checked out locally). Optional: `--env-from example|main|parent` to override the env strategy. Add **`--if-not-exists`** to make it idempotent (succeeds with `"already_exists": true` instead of exit `10` when the worktree is already there).
- **`wtm clean <branch> --force --output json`** — remove worktree and local branch (remote untouched). `--force` is mandatory in JSON mode. Idempotent: cleaning an absent worktree succeeds with `"already_absent": true`. If the cleaned worktree is the **parent** of others, those children would be orphaned (their `source_branch` points at the removed branch). In JSON/non-interactive mode the children are **left untouched** and listed under `orphaned_children` unless you pass **`--reparent-children`**, which reparents them onto the cleaned worktree's grandparent (its own `source_branch`, or the base branch as fallback) and lists them under `reparented`. In interactive text mode wtm instead shows a recap and asks for confirmation before reparenting.
...
- **`wtm sync <branch> [<branch>…] --output json`** / **`wtm sync --all --output json`** — rebase the **selected** worktrees onto their recorded parent (`source_branch`) in cascade. **In `--output json`/non-interactive mode you must pass branch arguments or `--all`** — with neither, it exits with a usage error (there is no picker to fall back to). Pass branch names (from `wtm list`) to target specific worktrees; pass `--all` to sync every managed worktree. Selecting the **base/main worktree** just fetches + fast-forwards the base (no rebase), and the base is always fetched + fast-forwarded first regardless. Then for each selected branch (parents before children): fetches + fast-forwards that branch from its own `origin/<branch>` (so commits merged into the remote elsewhere are pulled in), then rebases it onto its refreshed parent. With `--all`, a chain `main → feat → dev1/dev2` is updated in one shot; targeting a single branch rebases only it onto its (refreshed) parent. The cascade is **local** — pushing is separate. Per-branch `status` in the JSON: `synced` (rebased), `up_to_date`, `skipped_dirty` (uncommitted changes — its descendants become `skipped_ancestor`), `skipped_ancestor`, `diverged` (local **and** `origin/<branch>` carry commits the other lacks, by patch — a genuine conflict left untouched for manual reconcile; descendants skipped. A branch rebased locally in a prior run but not yet pushed is **not** diverged — its remote commits are already integrated, so it comes back as `up_to_date` and pushable), `conflict` (rebase auto-aborted by default, working tree left clean — needs manual resolution; descendants skipped. Under `--keep-conflict` the rebase is instead **left in progress** in the worktree, flagged by `kept_in_progress: true`), `rebase_in_progress` (a **prior** run already left a rebase paused in this worktree — its branch is recovered from the paused rebase; skipped until the user finishes it with `git rebase --continue`/`--abort`; descendants skipped), `error`, `unknown_parent` (no recorded parent). A branch argument that matches no worktree exits **`11`** (`branch not found`). Each step also reports `path` (the worktree directory), `old_tip`, `new_tip`, `onto_tip`, `commits_replayed`, `conflict_files` (the unmerged paths on a `conflict`, populated in both modes), `kept_in_progress` (the conflicting rebase was left in progress rather than aborted), `push_pending` (the branch is ahead of `origin/<branch>` and eligible for force-push — true for `synced` **and** for `up_to_date` branches still unpushed from an earlier run), and `pushed`. Exits non-zero if any `conflict`/`error`. Flags: `--all` (sync every worktree; cannot be combined with branch arguments), `--dry-run` (preview only — stays fully offline, no fetch/rebase/push), `--base <branch>` (override base), `--keep-conflict` (on conflict, leave the rebase in progress in its worktree for manual resolution instead of aborting; descendants are still skipped but **independent** branches keep syncing, so several worktrees may be left mid-rebase — each `conflict` step then has `kept_in_progress: true` and a populated `conflict_files`/`path`), `--push` (force-push every `push_pending` branch with `--force-with-lease` — **the only way to push in `--output json`/non-interactive mode**, since the grouped push confirmation can't prompt), `--no-push` (never push), `-y`/`--yes` (skip the pre-sync confirmation in text mode). Without `--push`/`--no-push` in JSON mode, nothing is pushed. In interactive text mode, running `wtm sync` with no arguments opens a multi-select picker of worktrees, then a second screen to choose whether conflicts abort or are kept in progress.
- **`wtm reparent <branch> --to <parent> --output json`** — change the recorded parent (`source_branch`) of a worktree, i.e. the branch `wtm sync` rebases it onto. **Metadata only** — no rebase happens until the next `wtm sync`. Use it for stacked branches: once a middle branch is merged (e.g. `feat → dev/a → dev/b`, `dev/a` merged into `feat`), reparent `dev/b` onto `feat`. The new parent must exist as a **local branch or a remote-tracking ref** (`origin/x`), and the resulting parent chain must stay acyclic (a self-parent or a cycle is rejected). JSON returns `{branch, old_parent, new_parent}`. **In `--output json`/non-interactive mode you must pass the worktree argument and `--to`** — with neither there is no picker to fall back to and it exits with a usage error. In interactive text mode, omitting them opens pickers for the worktree and the new parent.
...
- A `sync` exited non-zero → at least one branch has `status: conflict` or `error` in the JSON. For `conflict` (default mode), the rebase was aborted (working tree is clean), so the branch and its descendants were skipped — tell the user to resolve that branch manually (`git rebase <parent>` in its worktree), then re-run `wtm sync`. If `kept_in_progress: true` (ran with `--keep-conflict`), the rebase is **already in progress** in that step's `path`: the user resolves the files in `conflict_files` and runs `git rebase --continue` (or `git rebase --abort`) there directly — no need to restart the rebase. A `diverged` status (exit stays 0) means local and `origin/<branch>` each have commits the other lacks (by patch) — the user must reconcile that branch (`git pull --rebase` or a manual merge) before re-running. Re-running `sync` after a local-only rebase is safe: branches already rebased come back `up_to_date` with `push_pending: true` (push them with `--push`), not `diverged`.
